// Initialize when page loads
        document.addEventListener('DOMContentLoaded', function() {
            ...
            avatarEdit.addEventListener('click', function() {
                alert('در یک اپلیکیشن واقعی، این دکمه امکان آپلود تصویر اواتار را فراهم می‌کند.');
                
                // Simulate avatar upload
                const genders = ['male', 'female', 'neutral'];
                const randomGender = genders[Math.floor(Math.random() * genders.length)];
                
                // Update to random gender for demo
                document.querySelector(`input[name="gender"][value="${randomGender}"]`).checked = true;
                updateAvatarByGender(randomGender);
                
                console.log('Avatar updated (simulated)');
            });
